...
First, let's create a deployment of nginx, it’s called deploy.yaml. Select deploy.yaml to run the script. 
...
Now, let's run a service so that our nginx Deployment is accessible within the cluster. The YAML file is named service.yaml. Select service.yaml. 
Service.yaml
